...
(function(){
	...
	function tryDecompressDownload(arrayBufferIn) {
		return new Promise((resolve) => {
			...
			fetchStream((new Blob([arrayBufferIn])).stream().pipeThrough(ds).getReader()).then((arrayBufferOut) => {
				...
				console.error("не получилось декомпрессить");
				console.error(err);
				resolve(null);
			});
		});
	...
	function tryDownloadURL(ipfsURL) {
		...
		return new Promise((resolve) => {
			...
			xhr.addEventListener("progress", (evt) => {
				updateProgressBar("Обновление: " + Math.round(evt.loaded * 0.001) + " / " + Math.round(opts.dlSize * 0.001) + " kB", theIpfsURL, Math.min(evt.loaded / opts.dlSize, 1.0));
			});
			xhr.addEventListener("readystatechange", (evt) => {
				if(xhr.readyState === XMLHttpRequest.DONE) {
					updateProgressBar("Обновление: " + Math.round(opts.dlSize * 0.001) + " / " + Math.round(opts.dlSize * 0.001) + " kB", theIpfsURL, Math.min(evt.loaded / opts.dlSize, 1.0));
					...
					if(xhr.status === 200) {
						...
						console.error("Респонз код " + xhr.status + " для: " + theIpfsURL);
						resolve(null);
					}
				}
			});
			xhr.addEventListener("error", (evt) => {
				...
				currentXHR = null;
				console.warn("Не получилось скачать с линка " + theIpfsURL);
				resolve(null);
			});
			...
			xhr.addEventListener("abort", (evt) => {
				console.warn("Реквест сдох " + theIpfsURL);
				if(cancelButton !== null) {
					cancelButton.disabled = true;
				}
				currentXHR = null;
	...
	async function tryDownloadClient(ipfsCID, ipfsPath) {
		...
		for(var i = 0; i < IPFS_GATEWAYS.length; ++i) {
			...
			updateProgressBar("Обновление: 0 / " + Math.round(opts.dlSize * 0.001) + " кБ", url, 0.0);
			try {
				var j = await tryDownloadURL(url);
				if(j) {
					if(opts.gzip) {
						try {
							updateProgressBar("Распаковываем...", url, -1);
							j = await tryDecompressDownload(j);
							if(j) {
								return j;
							}else {
								throw "короче не получилось";
							}
						}catch(ex) {
							updateProgressBar("Не получилось декомпрессировать!", url, -1);
							console.warn("Не получилось бывает " + url);
							console.warn(ex);
						}
					}else {
						return j;
				...
				updateProgressBar("Не получилось скачать клиент!", url, 1.0);
				console.error("Возникла ошибка:  " + url);
				console.error(ex);
			}
	...
	function hasDownloadFailed(cidPath) {
		if(window.localStorage) {
			...
			if(keyValue) {
				try {
					if((Date.now() - parseInt(keyValue)) < 6 * 3600 * 1000) { // не обновляем
						return true;
					}else {
		...
		return new Promise((resolve) => {
			...
			openRequest.addEventListener("success", (evt2) => {
				...
				db.addEventListener("error", (err) => {
					console.error("не получилось загрузить с Idb!");
					console.error(err);
				});
	...
	window.addEventListener("load", async function() {
		if(!window.__eaglercraftLoaderClient) {
			console.error("не найден лоадер");
			return;
		}
		...
		initProgressScreen();
		updateProgressScreen("Загрузка " + opts.name);
		updateProgressBar("Пожалуйста, подождите...", "", -1);
		
		if(!window.indexedDB) {
			console.error("IDB не работет давайте скачаем");
			var dl = await tryDownloadClient(opts.cid, opts.path);
			if(dl) {
				updateProgressBar("Запуск!", "Последний запуск: сейчас", -1);
				await delayProgress(500);
				loadClientFile(dl);
			}else {
				updateProgressScreen("Не получилось скачать клиент!");
				updateProgressBar("Попробуйте позже.", "Не получилось скачать напрямую", -1);
			}
			return;
		}
		...
		if(cachedClient) {
			...
			if(hasFailed) {
				hasFailed = confirm("Не получилсь обновить клиент!\n\nХочешь использовать бекап " + clientDisplayAge + " дней назад?");
			}
			if(hasFailed || cachedClient.clientVersionUID === clientCIDPath) {
				if(hasFailed) {
					console.error("Внимание: обновить клиент не получилось, так что будем использовать старый.\n\n");
				}
				console.log("Найден клиент в кеше.");
				...
				console.log("Нашли клиент в кеше но он требует обновы");
			}
		}else {
			console.log("будем скачивать");
		}
		
		var dl = await tryDownloadClient(opts.cid, opts.path);
		if(dl) {
			updateProgressBar("Выполняем кеширование...", "Последний запуск: сейчас", -1);
			...
			if(cachedClient) {
				...
				if(confirm("Не получилсь обновить клиент!\n\nХочешь использовать бекап " + clientDisplayAge + " дней назад?")) {
					updateProgressBar("Запуск!", "Последний запуск: " + clientDisplayAge + " дней назад.", -1);
					await delayProgress(1500);
					loadClientFile(cachedClient.clientPayload);
					return;
			...
			updateProgressScreen("Ошибка: не получилось скачать клиент.");
			updateProgressBar("Попробуйте позже.", "Не удалось скачать!", -1);
		}
	});
			...
			if(s) {
				if(["flameddogo99-eaglermobile.js", "irv77-eaglercraft-mobile.js"].includes(s)) {
					if(checkNotMobileBrowser()) {
						if(confirm("В вашем брауере доступен Pointer Lock. \n\nОтключить touch mode?")) {
							q.delete("userscript");
							window.location.href = window.location.origin + window.location.pathname + (q.size > 0 ? ("?" + q.toString()) : "") + window.location.hash;
							return;
						}
					}
					alert("Внимание! Это может крашнуть вашу игру!");
					var scriptElement = document.createElement("script");
					scriptElement.type = "text/javascript";
